...
<xsl:template match="params">
	...
	<style>
		.cl-position-table { width: <xsl:value-of select="$width"/>px; height: <xsl:value-of select="$height"/>px; }
		.cl-position-table td { width: <xsl:value-of select="$width"/>px; height: <xsl:value-of select="$height"/>px; }
		.cl-position { color: <xsl:value-of select="$font-color-fixed"/>; font-size: <xsl:value-of select="font_size"/>px; font-family: <xsl:value-of select="$_font"/>;<xsl:value-of select="$_css_font_style"/><xsl:if test="text_shadow_strength!='none'">text-shadow: <xsl:value-of select="$_text_shadow"/>;</xsl:if> }
	</style>
	<div>
		<xsl:attribute name="class">clo-nav_0001_position</xsl:attribute>
		<div>
			<xsl:attribute name="class">cl-container</xsl:attribute>
			<table>
				<xsl:attribute name="class">cl-position-table</xsl:attribute>
				<tr>
					<td>
						<div class="position">
							<xsl:attribute name="class">cl-position</xsl:attribute>
							<xsl:choose>
								<xsl:when test="display_type='sub_module'">
									<xsl:variable name="A1">
										<xsl:choose>
											<xsl:when test="contains(text_sub_module, '{{N}}')">
												<xsl:value-of select="substring-before(text_sub_module,'{{N}}')" />N<xsl:value-of select="substring-after(text_sub_module,'{{N}}')" />
											</xsl:when>
											<xsl:otherwise><xsl:value-of select="text_sub_module" /></xsl:otherwise>
										</xsl:choose>
									</xsl:variable>
									<xsl:variable name="A2">
										<xsl:choose>
											<xsl:when test="contains($A1, '{{M}}')">
												<xsl:value-of select="substring-before($A1,'{{M}}')" />M<xsl:value-of select="substring-after($A1,'{{M}}')" />
											</xsl:when>
											<xsl:otherwise><xsl:value-of select="$A1" /></xsl:otherwise>
										</xsl:choose>
									</xsl:variable>
									<span class="cl-label" cl-role="position-text"><xsl:value-of select="$A2"/></span>
								</xsl:when>
								<xsl:when test="display_type='sub_chapter'">
									<xsl:variable name="B1">
										<xsl:choose>
											<xsl:when test="contains(text_sub_chapter, '{{X}}')">
												<xsl:value-of select="substring-before(text_sub_chapter,'{{X}}')" />X<xsl:value-of select="substring-after(text_sub_chapter,'{{X}}')" />
											</xsl:when>
											<xsl:otherwise><xsl:value-of select="text_sub_chapter" /></xsl:otherwise>
										</xsl:choose>
									</xsl:variable>
									<xsl:variable name="B2">
										<xsl:choose>
											<xsl:when test="contains($B1, '{{Y}}')">
												<xsl:value-of select="substring-before($B1,'{{Y}}')" />Y<xsl:value-of select="substring-after($B1,'{{Y}}')" />
											</xsl:when>
											<xsl:otherwise><xsl:value-of select="$B1" /></xsl:otherwise>
										</xsl:choose>
									</xsl:variable>
									<span class="cl-label" cl-role="position-text"><xsl:value-of select="$B2"/></span>
								</xsl:when>
								<xsl:when test="display_type='sub_all'">
									<xsl:variable name="C1">
										<xsl:choose>
											<xsl:when test="contains(text_sub_all, '{{X}}')">
												<xsl:value-of select="substring-before(text_sub_all,'{{X}}')" />X<xsl:value-of select="substring-after(text_sub_all,'{{X}}')" />
											</xsl:when>
											<xsl:otherwise><xsl:value-of select="text_sub_all" /></xsl:otherwise>
										</xsl:choose>
									</xsl:variable>
									<xsl:variable name="C2">
										<xsl:choose>
											<xsl:when test="contains($C1, '{{Y}}')">
												<xsl:value-of select="substring-before($C1,'{{Y}}')" />Y<xsl:value-of select="substring-after($C1,'{{Y}}')" />
											</xsl:when>
									...
									<xsl:variable name="C3">
										<xsl:choose>
											<xsl:when test="contains($C2, '{{N}}')">
												<xsl:value-of select="substring-before($C2,'{{N}}')" />N<xsl:value-of select="substring-after($C2,'{{N}}')" />
											</xsl:when>
									...
									<xsl:variable name="C4">
										<xsl:choose>
											<xsl:when test="contains($C3, '{{M}}')">
												<xsl:value-of select="substring-before($C3,'{{M}}')" />M<xsl:value-of select="substring-after($C3,'{{M}}')" />
											</xsl:when>
											<xsl:otherwise><xsl:value-of select="$C3" /></xsl:otherwise>
										</xsl:choose>
									</xsl:variable>
									<span class="cl-label" cl-role="position-text"><xsl:value-of select="$C4"/></span>
								</xsl:when>
								<xsl:otherwise>
									<span class="cl-label"><xsl:value-of select="position_text"/></span>
									<span class="cl-value" cl-role="slide-number">N</span>
									<span class="cl-divider"><xsl:value-of select="position_divider"/></span>
									<span class="cl-total" cl-role="slide-total">M</span>
								</xsl:otherwise>
							</xsl:choose>
						</div>
					</td>
