...
<xsl:template match="params">
	...
	<div class="style-custom" style="display: none">
		...
		<xsl:if test="imagefile!=''">
			...
			<div class="rule">
				...
				<span class="rule-static">position: relative; margin: 10px;</span>
				<xsl:if test="number(radius)!=0">
					<span class="rule-dynamic">
						<xsl:attribute name="data-type">border-radius</xsl:attribute>
						<xsl:attribute name="data-value"><xsl:value-of select="radius"/>px</xsl:attribute>
			...
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/>_ZOOM .cl-zoom-box img</xsl:attribute>
				<span class="rule-static">position: relative; <xsl:if test="hide_onclick='yes'">cursor: pointer;</xsl:if></span>
				<xsl:if test="number(radius)!=0">
					<span class="rule-dynamic">
						<xsl:attribute name="data-type">border-radius</xsl:attribute>
						<xsl:attribute name="data-value"><xsl:value-of select="radius"/>px</xsl:attribute>
    ...
    <div>
        ...
        <div>
			...
			<xsl:attribute name="class">cl-zoom-box</xsl:attribute>
			<img>
				<xsl:if test="hide_onclick='yes'"><xsl:attribute name="class">cl-close-on-img</xsl:attribute></xsl:if>
				<xsl:attribute name="id"><xsl:value-of select="$objectID"/>_ZOOM_IMG</xsl:attribute>
				<xsl:attribute name="src"><xsl:value-of select="imagefile"/></xsl:attribute>
				<xsl:attribute name="onload">CLO['<xsl:value-of select="$objectID"/>'].CallMethod({ sMethod: 'Adjust', oMethodArgs: { elem: this } })</xsl:attribute>
