...
<xsl:template match="params">
	...
	<xsl:variable name="main.sFontFamily">
		<xsl:choose>
	...
	<xsl:variable name="main.sFontShadow">
	...
	<xsl:variable name="main.sFontShadowCSS"><xsl:if test="text_shadow_strength!='none'">text-shadow: <xsl:value-of select="$main.sFontShadow"/>;</xsl:if></xsl:variable>
	<xsl:variable name="main.sFontCSS">color: <xsl:value-of select="$font-color-fixed" />; font-family: <xsl:value-of select="$main.sFontFamily"/>; font-size:<xsl:value-of select="font_size" />px;<xsl:choose><xsl:when test="font_style='bold'">font-weight: bold;</xsl:when><xsl:when test="font_style='italic'">font-style: italic;</xsl:when><xsl:when test="font_style='bolditalic'">font-weight: bold; font-style: italic;</xsl:when></xsl:choose><xsl:value-of select="$main.sFontShadowCSS"/></xsl:variable>
	<xsl:variable name="main.sRadiusString"><xsl:if test="number(border_radius)!=0"><xsl:value-of select="border_radius"/>px</xsl:if></xsl:variable>			  
	<xsl:variable name="main.sRadiusCSS"><xsl:if test="number(border_radius)!=0">-webkit-border-radius: <xsl:value-of select="$main.sRadiusString"/>; border-radius: <xsl:value-of select="$main.sRadiusString"/>;</xsl:if></xsl:variable>			  
	...
	<xsl:variable name="main.sShadowCSS"><xsl:if test="shadow!='none'">box-shadow: <xsl:value-of select="$main.sShadowString"/>;</xsl:if></xsl:variable>			  
	<xsl:variable name="main.sColor2">
		<xsl:call-template name="autogradient">
			<xsl:with-param name="color.base" select="$bg-color-fixed"/>
		</xsl:call-template>
	</xsl:variable>
	<xsl:variable name="main.sGradientCSS">background-image: linear-gradient(0deg, <xsl:value-of select="$bg-color-fixed"/> 0%, <xsl:value-of select="$main.sColor2"/> 100%);</xsl:variable>
	<style>
		.cl-btn { width: <xsl:value-of select="$width"/>px; height: <xsl:value-of select="$height"/>px; padding: <xsl:value-of select="padding"/>px; text-align: <xsl:value-of select="text_align"/>; border-style: <xsl:value-of select="border_style"/>; border-width: <xsl:value-of select="border_width"/>px; border-color: <xsl:value-of select="$border-color-fixed"/>; background-color: <xsl:value-of select="$bg-color-fixed"/>; <xsl:value-of select="$main.sFontCSS"/><xsl:value-of select="$main.sRadiusCSS"/><xsl:value-of select="$main.sGradientCSS"/><xsl:value-of select="$main.sShadowCSS"/> }
	</style>
	<div>
		<xsl:attribute name="class">cl-container</xsl:attribute>
		<button>
			<xsl:attribute name="id"><xsl:value-of select="$objectID"/>_BTN</xsl:attribute>
			<xsl:attribute name="class">cl-btn</xsl:attribute>
