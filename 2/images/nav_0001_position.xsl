...
<xsl:template match="params">
	...
	<div class="style-custom" style="display: none">
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-position-table</xsl:attribute>
			<span class="rule-static">width: <xsl:value-of select="$width"/>px; height: <xsl:value-of select="$height"/>px;</span>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-position-table td</xsl:attribute>
			<span class="rule-static">width: <xsl:value-of select="$width"/>px; height: <xsl:value-of select="$height"/>px;</span>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-position</xsl:attribute>
			<span class="rule-static">color: <xsl:value-of select="$font-color-fixed"/>; font-size: <xsl:value-of select="font_size"/>px; font-family: <xsl:value-of select="$_font"/>; <xsl:value-of select="$_css_font_style"/><xsl:if test="text_shadow_strength!='none'">text-shadow: <xsl:value-of select="$_text_shadow"/>;</xsl:if></span>
		</div>
	</div>
	<div>
					...
					<div class="position">
						...
						<xsl:choose>
							<xsl:when test="display_type='sub_module'">
								<span class="cl-label" cl-role="position-text"><xsl:value-of select="text_sub_module"/></span>
							</xsl:when>
							<xsl:when test="display_type='sub_chapter'">
								<span class="cl-label" cl-role="position-text"><xsl:value-of select="text_sub_chapter"/></span>
							</xsl:when>
							<xsl:when test="display_type='sub_all'">
								<span class="cl-label" cl-role="position-text"><xsl:value-of select="text_sub_all"/></span>
							</xsl:when>
							<xsl:otherwise>
